...
```python
from dfcc_rates_docs import DfccRatesDocsClient, iter_lab_endpoint

with DfccRatesDocsClient() as client:
    for page in iter_lab_endpoint(client, None, max_pages=3, page_size=50):
        print(page.page, len(page.items), page.done)
```

```ts
import { DfccRatesDocsClient, iterLabEndpoint } from "./src/index.js";
const client = new DfccRatesDocsClient();
...
```python
from dfcc_rates_docs import shard_groups, shard_page_numbers, shard_offsets
